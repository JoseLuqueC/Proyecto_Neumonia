...
- [ ] **Sin Warnings:** La ejecución de `make run` o los tests corre limpia **sin advertencias (warnings)**.
- [ ] **Control de Exclusiones (`.gitignore`):** Los modelos pesados (`conv_MLP_84.h5`) y datasets **NO** están rastreados por Git.
- [ ] **Estructura del Código:** El código fuente está dentro de `src/` y las pruebas dentro de `test/`.
- [ ] **Pruebas Unitarias (`pytest`):** Se ejecutó `make test` y todas las pruebas pasaron correctamente.
...
<!--- 
Adjunta capturas de pantalla, logs o salidas del comando `make test` o ejecución en Docker demostrando que el código funciona.
-->
